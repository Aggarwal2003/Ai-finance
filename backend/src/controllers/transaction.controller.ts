...
export const scanReceiptController = asyncHandler(
  async (req: Request, res: Response) => {
    console.log("1️⃣ Scan route hit");

    const file = req.file;
    console.log("2️⃣ File exists:", !!file);

    if (file) {
      console.log("3️⃣ File path:", file.path);
    }

    console.log("4️⃣ Calling scanReceiptService");
    const result = await scanReceiptService(file);

    console.log("5️⃣ Service returned:", result);
